...
from starVLA.model.tools import read_mode_config,read_model_config_from_run
...
class ModelClient:
    ...
    def step(
        ...
        **kwargs
        ...
        return {"raw_actions": self.raw_actions}
    ...
    def unnormalize_actions(normalized_actions: np.ndarray, action_norm_stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        normalized_actions[:, 7] = np.where(normalized_actions[:, 7] < 0.0, 0, 1) 
        normalized_actions[:, 15] = np.where(normalized_actions[:, 15] < 0.0, 0, 1)
        actions = np.where(
            mask,
    ...
    def get_action_stats(unnorm_key: str, policy_ckpt_path) -> dict:
        ...
        model_config, norm_stats = read_model_config_from_run(policy_ckpt_path)  # read config and norm_stats
    ...
    def get_action_chunk_size(policy_ckpt_path):
        model_config, _ = read_model_config_from_run(policy_ckpt_path)  # read config and norm_stats
